...
function depositToken0(token0, slippage, token0Reserve, token1Reserve, totalLpTokens) {
  ...
  const maxToken0 = token0.mul(slippageFactor).div(new BN3(1e9));
  const maxToken1 = token1.mul(slippageFactor).div(new BN3(1e9));
...
function ceilDiv2(numerator, denominator) {
  return numerator.add(denominator).sub(new BN3(1)).div(denominator);
}
function depositLpToken(lpToken, slippage, baseReserve, quoteReserve, totalLpTokens) {
  ...
  const slippageFactor = new BN3((1 + slippage / 100) * 1e9);
  const slippageDenominator = new BN3(1e9);
...
function buyBaseInput({
  ...
  baseReserve,
  quoteReserve,
  virtualQuoteReserves = new BN5(0),
  globalConfig,
  baseMintAccount,
  baseMint,
  coinCreator,
  ...
  const coinCreatorFee = PublicKey3.default.equals(coinCreator) ? new BN5(0) : fee(quoteAmountIn, coinCreatorFeeBps);
...
function buyQuoteInput({
  ...
  const totalFeeBps = lpFeeBps.add(protocolFeeBps).add(PublicKey3.default.equals(coinCreator) ? new BN5(0) : coinCreatorFeeBps);
  const denominator = new BN5(1e4).add(totalFeeBps);
  let effectiveQuote = quote.mul(new BN5(1e4)).div(denominator);
  const lpFee = fee(effectiveQuote, lpFeeBps);
  const protocolFee = fee(effectiveQuote, protocolFeeBps);
  const coinCreatorFee = PublicKey3.default.equals(coinCreator) ? new BN5(0) : fee(effectiveQuote, coinCreatorFeeBps);
...
function sellBaseInput({
  ...
  baseReserve,
  quoteReserve,
  virtualQuoteReserves = new BN6(0),
  globalConfig,
  baseMintAccount,
  baseMint,
  coinCreator,
  ...
  const coinCreatorFee = PublicKey4.default.equals(coinCreator) ? new BN6(0) : fee(quoteAmountOut, coinCreatorFeeBps);
...
function sellQuoteInput({
  ...
  const rawQuote = calculateQuoteAmountOut(
    ...
    lpFeeBps,
    protocolFeeBps,
    PublicKey4.default.equals(coinCreator) ? new BN6(0) : coinCreatorFeeBps
  );
...
var PumpAmmSdk = class {
    ...
    return await this.withFixPoolInstructions(
      ...
      async () => {
        ...
        if (!this.accountExists(userBaseAccountInfo, baseTokenProgram)) {
          ...
          if (baseMint.equals(NATIVE_MINT3)) {
  ...
  async buyInstructionsNoPool(swapSolanaState, baseOut, maxQuoteIn) {
    ...
    return this.withWsolAccount(
      ...
      async () => {
        ...
        if (!pool.coinCreator.equals(PublicKey5.default)) {
          remainingAccounts.push({
            pubkey: poolV2PdaKey,
            isWritable: false,
            isSigner: false
  ...
  async collectCoinCreatorFee(collectCoinCreatorFeeSolanaState, payer = void 0) {
    ...
    const shouldCloseCoinCreatorATA = quoteMint.equals(NATIVE_MINT3) && coinCreator.equals(actualPayer);
    const coinCreatorTokenAccountExists = this.accountExists(
      coinCreatorTokenAccountInfo,
      quoteTokenProgram
    );
    const coinCreatorTokenAccountIsAta = coinCreatorTokenAccount.equals(
      getAssociatedTokenAddressSync2(
        quoteMint,
        coinCreator,
        true,
        quoteTokenProgram
...
var PumpAmmAdminSdk = class {
   ...
   */
  async adminSetCoinCreator(mint, newCoinCreator, quoteMint = NATIVE_MINT4) {
  ...
  async adminUpdateTokenIncentives(startTime, endTime, dayNumber, tokenSupplyPerDay, secondsInADay = new BN8(86400), mint = PUMP_MINT, tokenProgram = TOKEN_2022_PROGRAM_ID3) {
    const { admin } = await this.fetchGlobalConfigAccount();
...
function totalUnclaimedTokens(globalVolumeAccumulator, userVolumeAccumulator, currentTimestamp = Date.now() / 1e3) {
  ...
  let currentTimestampBn = new BN9(currentTimestamp);
...
var OnlinePumpAmmSdk = class {
  ...
  async resolveQuote(quoteMint, quoteTokenProgram) {
    ...
    if (quoteMint.equals(NATIVE_MINT5)) {
      return { quoteMint, quoteTokenProgram: TOKEN_PROGRAM_ID2 };
    }
    ...
    if (!owner.equals(TOKEN_PROGRAM_ID2) && !owner.equals(TOKEN_2022_PROGRAM_ID4)) {
      throw new Error(
   ...
   */
  async collectCoinCreatorFeeSolanaState(coinCreator, coinCreatorTokenAccount = void 0, quoteMint = NATIVE_MINT5, quoteTokenProgram = void 0) {
   ...
   */
  async getCoinCreatorVaultBalance(coinCreator, quoteMint = NATIVE_MINT5, quoteTokenProgram = void 0) {
    ({ quoteMint, quoteTokenProgram } = await this.resolveQuote(
      quoteMint,
      quoteTokenProgram
    ));
  ...
  async claimTokenIncentives(user, payer) {
    ...
    if (mint.equals(PublicKey6.default)) {
      return [];
    }
