...
function buyBaseInput({
  ...
  const coinCreatorFee = PublicKey3.default.equals(coinCreator) ? new BN6(0) : fee(quoteAmountIn, coinCreatorFeeBps);
...
function buyQuoteInput({
  ...
  const totalFeeBps = lpFeeBps.add(protocolFeeBps).add(PublicKey3.default.equals(coinCreator) ? new BN6(0) : coinCreatorFeeBps);
  const denominator = new BN6(1e4).add(totalFeeBps);
  let effectiveQuote = quote.mul(new BN6(1e4)).div(denominator);
  const lpFee = fee(effectiveQuote, lpFeeBps);
  const protocolFee = fee(effectiveQuote, protocolFeeBps);
  const coinCreatorFee = PublicKey3.default.equals(coinCreator) ? new BN6(0) : fee(effectiveQuote, coinCreatorFeeBps);
...
function sellBaseInput({
  ...
  baseReserve,
  quoteReserve,
  virtualQuoteReserves = new BN7(0),
  globalConfig,
  baseMintAccount,
  baseMint,
  coinCreator,
  ...
  const coinCreatorFee = PublicKey4.default.equals(coinCreator) ? new BN7(0) : fee(quoteAmountOut, coinCreatorFeeBps);
...
function sellQuoteInput({
  ...
  const rawQuote = calculateQuoteAmountOut(
    ...
    lpFeeBps,
    protocolFeeBps,
    PublicKey4.default.equals(coinCreator) ? new BN7(0) : coinCreatorFeeBps
  );
...
var OnlinePumpAmmSdk = class {
  ...
  async resolveQuote(quoteMint, quoteTokenProgram) {
    ...
    if (quoteMint.equals(NATIVE_MINT4)) {
    ...
    if (!owner.equals(TOKEN_PROGRAM_ID2) && !owner.equals(TOKEN_2022_PROGRAM_ID3)) {
      throw new Error(
   ...
   */
  async collectCoinCreatorFeeSolanaState(coinCreator, coinCreatorTokenAccount = void 0, quoteMint = NATIVE_MINT4, quoteTokenProgram = void 0) {
   ...
   */
  async getCoinCreatorVaultBalance(coinCreator, quoteMint = NATIVE_MINT4, quoteTokenProgram = void 0) {
    ({ quoteMint, quoteTokenProgram } = await this.resolveQuote(
      quoteMint,
      quoteTokenProgram
    ));
