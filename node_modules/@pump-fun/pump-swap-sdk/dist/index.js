...
var pump_amm_default = {
  ...
  instructions: [
    ...
    {
      name: "admin_set_coin_creator_fee_editable",
      discriminator: [
        204,
        65,
        86,
  ...
  events: [
    ...
    {
      name: "AdminSetCoinCreatorFeeEditableEvent",
      discriminator: [
        191,
        198,
        127,
    ...
    {
      name: "SetCoinCreatorFeeBpsEvent",
      discriminator: [
        230,
        138,
        241,
...
var PumpAmmSdk = class {
  ...
  async createPoolInstructions(createPoolSolanaState, baseIn, quoteIn, {
    creatorFeeBps = new import_bn7.default(0),
      ...
      async () => {
        ...
        instructions.push(
          await this.offlineProgram.methods.createPool(
            ...
            baseIn,
            quoteIn,
            import_web35.SystemProgram.programId,
            false,
            { 0: false },
            { 0: creatorFeeBps },
            { 0: canEditCreatorFee }
  ...
  async buyInstructionsNoPool(swapSolanaState, baseOut, maxQuoteIn) {
    ...
    return this.withWsolAccount(
      ...
      async () => {
        ...
        if (!pool.coinCreator.equals(import_web35.PublicKey.default)) {
          remainingAccounts.push({
            pubkey: poolV2PdaKey,
            isWritable: false,
            isSigner: false
  ...
  async collectCoinCreatorFee(collectCoinCreatorFeeSolanaState, payer = void 0) {
    ...
    const shouldCloseCoinCreatorATA = quoteMint.equals(import_spl_token3.NATIVE_MINT) && coinCreator.equals(actualPayer);
    const coinCreatorTokenAccountExists = this.accountExists(
      coinCreatorTokenAccountInfo,
      quoteTokenProgram
    );
    const coinCreatorTokenAccountIsAta = coinCreatorTokenAccount.equals(
      (0, import_spl_token3.getAssociatedTokenAddressSync)(
        quoteMint,
        coinCreator,
        true,
        quoteTokenProgram
  ...
  async setCoinCreatorFeeBpsInstruction({
    ...
    return this.offlineProgram.methods.setCoinCreatorFeeBps(creatorFeeBps).accountsPartial({
      coinCreator,
      globalConfig: GLOBAL_CONFIG_PDA,
      pool
    }).remainingAccounts(
      sharingConfig === void 0 ? [] : [{ pubkey: sharingConfig, isWritable: false, isSigner: false }]
    ).instruction();
  }
  /**
  ...
  async adminSetCoinCreatorFeeEditableInstruction({
    ...
    return this.offlineProgram.methods.adminSetCoinCreatorFeeEditable().accountsPartial({
      adminSetCoinCreatorAuthority,
      globalConfig: GLOBAL_CONFIG_PDA,
      pool
    }).instruction();
...
var PumpAmmAdminSdk = class {
   ...
   */
  async adminSetCoinCreator(mint, newCoinCreator, quoteMint = import_spl_token4.NATIVE_MINT) {
    ...
    return this.program.methods.adminSetCoinCreator(newCoinCreator).accountsPartial({
      pool: canonicalPumpPoolPda(mint, quoteMint),
  ...
  async adminUpdateTokenIncentives(startTime, endTime, dayNumber, tokenSupplyPerDay, secondsInADay = new import_bn8.default(86400), mint = PUMP_MINT, tokenProgram = import_spl_token4.TOKEN_2022_PROGRAM_ID) {
    const { admin } = await this.fetchGlobalConfigAccount();
...
function totalUnclaimedTokens(globalVolumeAccumulator, userVolumeAccumulator, currentTimestamp = Date.now() / 1e3) {
  ...
  let currentTimestampBn = new import_bn9.default(currentTimestamp);
...
var OnlinePumpAmmSdk = class {
  ...
  async createPoolSolanaState(index, creator, baseMint, quoteMint, userBaseTokenAccount = void 0, userQuoteTokenAccount = void 0) {
    ...
    const poolBaseTokenAccount = (0, import_spl_token5.getAssociatedTokenAddressSync)(
      baseMint,
      poolKey,
      true,
      baseTokenProgram
    );
    const poolQuoteTokenAccount = (0, import_spl_token5.getAssociatedTokenAddressSync)(
      quoteMint,
      poolKey,
      true,
      quoteTokenProgram
    ...
    if (userBaseTokenAccount === void 0) {
      userBaseTokenAccount = (0, import_spl_token5.getAssociatedTokenAddressSync)(
        baseMint,
        creator,
        true,
        baseTokenProgram
    ...
    if (userQuoteTokenAccount === void 0) {
      userQuoteTokenAccount = (0, import_spl_token5.getAssociatedTokenAddressSync)(
        quoteMint,
        creator,
        true,
        quoteTokenProgram
  ...
  async swapSolanaState(poolKey, user, userBaseTokenAccount = void 0, userQuoteTokenAccount = void 0) {
    ...
    const decodedBaseMintAccount = import_spl_token5.MintLayout.decode(baseMintAccountInfo.data);
    if (quoteMintAccountInfo === null) {
      throw new Error(`quoteMint=${quoteMint.toString()} not found`);
    }
    ...
    const decodedPoolBaseTokenAccount = import_spl_token5.AccountLayout.decode(
      poolBaseAccountInfo.data
    );
    const decodedPoolQuoteTokenAccount = import_spl_token5.AccountLayout.decode(
  ...
  async liquiditySolanaState(poolKey, user, userBaseTokenAccount = void 0, userQuoteTokenAccount = void 0, userPoolTokenAccount = void 0) {
    ...
    if (userPoolTokenAccount === void 0) {
      userPoolTokenAccount = (0, import_spl_token5.getAssociatedTokenAddressSync)(
        lpMint,
        user,
        true,
        import_spl_token5.TOKEN_2022_PROGRAM_ID
      );
    }
  ...
  async resolveQuote(quoteMint, quoteTokenProgram) {
    ...
    if (quoteMint.equals(import_spl_token5.NATIVE_MINT)) {
      return { quoteMint, quoteTokenProgram: import_spl_token5.TOKEN_PROGRAM_ID };
    }
    ...
    if (!owner.equals(import_spl_token5.TOKEN_PROGRAM_ID) && !owner.equals(import_spl_token5.TOKEN_2022_PROGRAM_ID)) {
      throw new Error(
   ...
   */
  async collectCoinCreatorFeeSolanaState(coinCreator, coinCreatorTokenAccount = void 0, quoteMint = import_spl_token5.NATIVE_MINT, quoteTokenProgram = void 0) {
    ...
    if (coinCreatorTokenAccount === void 0) {
      coinCreatorTokenAccount = (0, import_spl_token5.getAssociatedTokenAddressSync)(
        quoteMint,
        coinCreator,
        true,
        quoteTokenProgram
   ...
   */
  async getCoinCreatorVaultBalance(coinCreator, quoteMint = import_spl_token5.NATIVE_MINT, quoteTokenProgram = void 0) {
    ({ quoteMint, quoteTokenProgram } = await this.resolveQuote(
      quoteMint,
      quoteTokenProgram
    ));
  ...
  vaultAtaBalance(vaultAta, accountInfo, tokenProgram) {
    ...
    try {
      return new import_bn10.default(
        (0, import_spl_token5.unpackAccount)(vaultAta, accountInfo, tokenProgram).amount.toString()
      );
    } catch (e) {
  ...
  /**
   * `PumpAmmSdk.setCoinCreatorFeeBpsInstruction` for `poolKey`, signed by `coinCreator`: the
   * pool's coin creator, or the admin of the coin's pump-fees SharingConfig when that PDA is the
   * coin creator (the config is then passed as the remaining account). With the GlobalConfig and
   * the pool at hand it rejects what the program would reject: the gate off
   * (`CreatorFeeNotConfigurable`), a permissionless pool
   * (`OnlyCanonicalPumpPoolsCanHaveCoinCreator`), a cashback pool
   * (`CreatorFeeNotAllowedForCashbackCoin`) and a rate outside
   * `1..=maxConfigurableCreatorFeeBps` (`CreatorFeeBpsOutOfRange`). Editability
   * (`Pool.canEditCreatorFee`, `CreatorFeeNotEditable`) and the signer's authority
   * (`NotAuthorized`) are left to the program: both may legitimately be changed by an earlier
   * instruction of the same transaction (the CTO flip, `admin_set_coin_creator`), which the pool
   * state read here would wrongly reject.
   */
  async setCoinCreatorFeeBpsInstructions(coinCreator, poolKey, creatorFeeBps) {
    const [globalConfigAccountInfo, poolAccountInfo] = await this.connection.getMultipleAccountsInfo([
      GLOBAL_CONFIG_PDA,
      poolKey
    ]);
    ...
    const pool = PUMP_AMM_SDK.decodePool(poolAccountInfo);
    if (!globalConfig.creatorFeeConfigurable) {
      throw new Error(
        "Configurable creator fees are disabled (GlobalConfig.creatorFeeConfigurable is false)"
      );
    }
    if (!isPumpPool(pool.baseMint, pool.creator)) {
      throw new Error(
        `pool=${poolKey.toString()} is not a canonical pump pool; only canonical pump pools can have a creator fee`
      );
    }
    if (pool.isCashbackCoin) {
      throw new Error(
        `pool=${poolKey.toString()} is a cashback coin; cashback coins cannot have a creator fee`
      );
    }
    if (creatorFeeBps.ltn(1) || creatorFeeBps.gt(globalConfig.maxConfigurableCreatorFeeBps)) {
      throw new Error(
        `creatorFeeBps=${creatorFeeBps.toString()} must be between 1 and GlobalConfig.maxConfigurableCreatorFeeBps=${globalConfig.maxConfigurableCreatorFeeBps.toString()}`
      );
    }
    const sharingConfig = feeSharingConfigPda(pool.baseMint);
    return [
      await PUMP_AMM_SDK.setCoinCreatorFeeBpsInstruction({
        coinCreator,
        pool: poolKey,
        creatorFeeBps,
        sharingConfig: pool.coinCreator.equals(sharingConfig) ? sharingConfig : void 0
      })
    ];
  }
  /**
   * `PumpAmmSdk.adminSetCoinCreatorFeeEditableInstruction` for `poolKey`, signed by
   * `GlobalConfig.adminSetCoinCreatorAuthority` (read from the account).
   */
  async adminSetCoinCreatorFeeEditableInstructions(poolKey) {
    const { adminSetCoinCreatorAuthority } = await this.fetchGlobalConfigAccount();
    return [
      await PUMP_AMM_SDK.adminSetCoinCreatorFeeEditableInstruction({
        adminSetCoinCreatorAuthority,
        pool: poolKey
      })
    ];
  ...
  async claimTokenIncentives(user, payer) {
    ...
    if (mint.equals(import_web36.PublicKey.default)) {
      return [];
    }
