...
function buyBaseInput({
  ...
  const coinCreatorFee = import_web33.PublicKey.default.equals(coinCreator) ? new import_bn3.default(0) : fee(quoteAmountIn, coinCreatorFeeBps);
...
function buyQuoteInput({
  ...
  const totalFeeBps = lpFeeBps.add(protocolFeeBps).add(import_web33.PublicKey.default.equals(coinCreator) ? new import_bn3.default(0) : coinCreatorFeeBps);
  const denominator = new import_bn3.default(1e4).add(totalFeeBps);
  let effectiveQuote = quote.mul(new import_bn3.default(1e4)).div(denominator);
  const lpFee = fee(effectiveQuote, lpFeeBps);
  const protocolFee = fee(effectiveQuote, protocolFeeBps);
  const coinCreatorFee = import_web33.PublicKey.default.equals(coinCreator) ? new import_bn3.default(0) : fee(effectiveQuote, coinCreatorFeeBps);
