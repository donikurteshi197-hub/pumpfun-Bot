...
/**
 * The pump `QuoteControl` PDA (seeds `[b"quote-control"]`), which lists the
 * quote mints `create_v2` accepts beyond SOL and the `Global` whitelist.
 */
export const QUOTE_CONTROL_PDA = pumpPda([Buffer.from("quote-control")]);
...
/**
 * The per-mint `holder-rewards` PDA (`["holder-rewards", mint]` under the pump
 * program): the creator of a holder-reward coin. Every creator fee of such a
 * coin accrues to `creatorVaultPda(holderRewardsPda(mint))` (and, after
 * graduation, to the pump-amm coin-creator vault of the same key); the
 * permissionless `collect_creator_fee*` move the vault onto the PDA and
 * `distribute_fee_to_holders` pays it out from there.
 */
...
/**
 * Normalizes a coin's quote mint to a usable mint address.
 *
 * Legacy SOL-quote coins store `PublicKey.default` (all zeros) as the
 * `quoteMint` on their bonding curve, so raw `bondingCurve.quoteMint` values
 * must be normalized before being used for PDA derivation. Maps
 * `undefined`/`null`/`PublicKey.default` to `NATIVE_MINT` and returns any
 * other quote mint unchanged.
 */
